    ...
    <div className="flex h-[calc(100vh-7rem)] min-h-[620px] flex-col overflow-hidden rounded-lg border border-border/70 bg-card shadow-sm">
      <div className="border-b border-border/70 px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold">{t("bin")}</h1>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">Deleted messages are shown here.</p>
      </div>
          ...
          <div className="flex min-h-full flex-col">
            <div className="min-h-0 flex-1 overflow-y-auto">
              <EmailList
                ...
                onOpen={(email) => {
                  selectEmail(email);
                  navigate(`/email/${email.id}`, { state: { from: "/bin" } });
                }}
